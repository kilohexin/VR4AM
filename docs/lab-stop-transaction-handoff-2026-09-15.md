...
主机端全量后端测试为 **773 passed / 0 failed**。下方实验室指定子集已在主机端另行执行，结果 **171 passed / 0 failed，退出码 0**。两者范围不同，不混用数量。
...
正常预期：**171 passed / 0 failed**；`process.json` 中 `status=EXITED`、`child_exit_code=0`、`observer_errors=[]`。观察器退出码与子进程退出码分别记录。若失败，保留原始失败，不反复运行直到通过，不删除失败日志。
