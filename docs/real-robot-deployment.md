...
当前生产策略仅自动请求 `stop_move`，不再因停止未确认而自动升级调用 `stop_sys`。若停止确认失败，后端仍保持故障锁定并拒绝新运动；`stop_move` 迟到返回不等于机械臂已停稳。若机械臂仍在运动，现场必须按原厂安全规程使用独立于本软件链路的停止手段，不得依赖网页刷新或等待软件自动补救。历史轮次曾记录 `stop_sys` 后非预期位移，该事实不代表当前策略仍会自动调用它；也不得把 `STOP` 状态字或 `target−actual` 归零单独当作物理静止的证明。
