...
> **仿真与真机隔离：**以下高响应速度只用于 `LEBAI_FAKE` 仿真。真机使用独立本地 YAML 中的保守参数，不会继承仿真调节值。
...
按下 Grip 的瞬间，系统同时记录当前实际手柄位姿和实际 TCP 位姿。仿真平移默认按 `1.5:1` 映射到 TCP，可在 PC 页面“操作设置”中以 `0.1` 步进调节到 `0.5:1–2.0:1`；手柄的 roll、pitch、yaw 固定按 `1:1` 映射到夹爪姿态。比例只能在 Grip 松开、仿真后端为 `IDLE` 且没有自动演练时修改，后端确认后才生效并保存为浏览器偏好。真机比例只读显示，来自 `real-robot.local.yaml`。

当前控制、状态回传和仿真 PVAT 频率均为 `50 Hz`。模拟器限制如下：
...
PC 页面按住鼠标右键拖动可环绕观察机械臂，点击“复位视角”恢复默认相机；鼠标左键和滚轮仍分别用于末端平面移动与深度移动。
