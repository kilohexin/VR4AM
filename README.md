...
- 右 Grip：运动离合。每次按下都会锁定当前“手柄—TCP”零位，此后手柄相对零位的前后、左右、上下和旋转直接映射到末端；松开即停止；
...
4. 分别做手柄俯仰、偏航、滚转，确认夹爪旋转符合直觉。
5. 在停止或锁定状态下，用左摇杆调整工作台前后左右，用左 Grip + 摇杆 Y 调整高度，并按下摇杆复位位置。
6. 将目标移动到相对工作边界，确认只出现琥珀色提示和一次短振动，不进入故障。
7. 将手柄退回有效区域，确认机械臂自动恢复跟随，无需点击复位。
8. 触发一个真正硬故障，确认系统停止并保持锁定。
9. 松开右 Grip 后点击复位，确认能看到“确认停止—返回 Home—确认稳定”的过程，完成后才可重新解锁。
10. 确认项目没有物体动力学、Cannon-es 或浏览器物理仿真行为。
